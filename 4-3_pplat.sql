...
drop table if exists public.kentran_4_3_table_pplat ;
...
select ce.icustay_id
	,min(ce.charttime)
from chartevents ce
where ce.itemid=224696 --  Plateau pressure
...
AND ce.itemid=224696 --  Plateau pressure
and ce.icustay_id in (select icustay_id from public.kentran_1_3_demographics_nockd) -- Ken: speed up by only querying our cohort 
...
select tab1.icustay_id
	, tab1.day
	, avg(tab1.valuenum) as avg_pplat
	,  max(tab1.valuenum) as max_pplat
	, sum((tab1.valuenum*(extract(hour from (tab1.charttime-tab1.prev_date))))) as pplat_time
...
select * into public.kentran_4_3_table_pplat 
from tab2 order by tab2.icustay_id, tab2.day;

Select * from public.kentran_4_3_table_pplat; 
