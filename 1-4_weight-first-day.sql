...
drop MATERIALIZED VIEW if exists weightfirstday;

DROP table IF EXISTS public.kentran_1_4_weightfirstday CASCADE;
CREATE table public.kentran_1_4_weightfirstday as
...
(
    select
        ie.icustay_id
        , 0.453592*percentile_cont(0.5) WITHIN GROUP (ORDER BY weight) as Weight_EchoInHosp -- convert from lb to kg
    from public.kentran_echoDATA ec -- change to run on other computer: echodata ec
    inner join icustays ie
...
-- Ken: we can just filter by 1-2 demograpic_select table, which was already filered for eligibility
from public.kentran_1_2_demographics_select demSe
...
-- admission weight
left join ce
    on demSe.icustay_id = ce.icustay_id

-- daily weights
left join dwt
    on demSe.icustay_id = dwt.icustay_id

-- in-hospital echo weight
left join echo_hadm eh
    on demSe.icustay_id = eh.icustay_id

-- pre-hospitalization echo weights
left join echo_nohadm enh
    on demSe.icustay_id = enh.icustay_id
order by demse.subject_id;

select * from public.kentran_1_4_weightfirstday
